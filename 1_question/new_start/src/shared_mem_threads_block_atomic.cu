...
*/
__global__ 
void step_func_atomic(double* d_result, double step_c, long num_steps, long num_steps_per_thread) {
	// shared s data per block
    extern __shared__ double  s_data[]; 
    double x = 0.0;
    unsigned int tid = threadIdx.x;  
...
__global__  void sum_blocks(double* d_result, double* final_result, unsigned int n){
	extern __shared__ double s_data[]; 
	unsigned int tid = threadIdx.x; 
	unsigned const int size = ceil(n/(double)blockDim.x);
	unsigned int i = 0;  
	s_data[tid] = 0;
...
//	cudaMemcpy(d_result, h_result, n_blocks* sizeof(double), cudaMemcpyHostToDevice); 
	...
	if( strcmp( mode , "kernel" ) == 0 ) {
		...
		int threads_sum_blocks_kernel =  (n_blocks>1024) ? 1024 : n_blocks;
		sum_blocks<<<1,threads_sum_blocks_kernel, threads_sum_blocks_kernel* sizeof(double)>>>(d_result, final_result, n_blocks);
	} else if( strcmp( mode , "atomic" ) == 0 ) {
		step_func_atomic<<<n_blocks, num_threads_per_block, num_threads_per_block  * sizeof(double) >>>(final_result, step, num_steps, num_steps_per_thread); 
	}
