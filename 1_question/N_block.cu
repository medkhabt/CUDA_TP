...
int main (int argc, char** argv)
{
      ...
      for ( int i = 0; i < argc; i++ ) {
	    ...
	    long num_blocks_min = (( __NUM_STEPS__ + num_blocks - 1) / 1024) + 1;  
...
// we have the num of steps	
    ...
    *result = 0.0; 
    ...
    long threads = (__NUM_STEPS__ + num_blocks -1 ) / num_blocks; 
    atomic_pi_without_step<<<num_blocks, threads>>>(d_steps, d_result); 
