...
__global__ void atomic_pi_thread_multiple_iterations(double *A, double *result, const long L) {
...
int main (int argc, char** argv)
{
      ...
      for ( int i = 0; i < argc; i++ ) {
	    ...
	    L = atol( argv[++i] ); 
	    printf("number of iterations per thread is : %ld \n", L); 
	    long minL =  (__NUM_STEPS__ + 1023) / (1024 * max_blocks);  
	    if(L < minL) L = minL; 
...
// we have the num of steps	
    ...
    *result = 0.0; 
    ...
    long num_blocks = ( num_steps + 1023 )  / (1024 * L);
    if(num_blocks <= 1)
	num_blocks = 1; 
    ...
    long threads = ((num_blocks > 1) ? 1024 : __NUM_STEPS__); 
    atomic_pi_thread_multiple_iterations<<<num_blocks, threads>>>(d_steps, d_result, L); 
    ...
    // pi = step * sum 
    // TODO I added L to fix teh result , but I am doing something wrong here. i need to check the amount of threads generated. 
// and the index inside the kernel function
    *result = (1.0/__NUM_STEPS__) * (*result / L);
