...
__global__ void reduce_without_s_mem(double *g_idata, double *g_odata, unsigned int n, const double step, unsigned int calculate_flag = 1) {    
	...
	if(i < n) {
	    ...
	    sdata[tid] = (double)4.0/(double)(1.0+x*x); 
	    //g_idata[i] = 1;
	}
	__syncthreads(); 
    }
    if(tid == 0 ) {
	printf("the sdata for the block %d for the first thread is : %lf \n", blockIdx.x, sdata[tid]); 
    }
...
// TODO free the h_step_mult
 ...
 // DECLARATION AND INITIALIZATION
    ...
    double *h_steps = (double *)malloc( num_steps * sizeof(double)); 


    *step = 1.0/(double) num_steps;
    std::fill_n(h_steps,num_steps, 0); 
    std::fill_n(h_block_result, num_blocks, 0);
    printf("the value of the first element of the array before : %lf. \n", h_steps[0]); 

    // CUDA ALLOC.
    cudaMalloc((void **)&d_steps, num_steps* sizeof(double)); 
    cudaMalloc((void **)&d_block_result, num_blocks * sizeof(double)); 
    cudaMalloc((void **)&d_result, 2 * sizeof(double)); 
    // CUDA CPY
    cudaMemcpy(d_steps, h_steps, num_steps * sizeof(double), cudaMemcpyHostToDevice); 
    cudaMemcpy(d_block_result, h_block_result, num_blocks * sizeof(double), cudaMemcpyHostToDevice); 
    cudaMemcpy(d_result, h_result, 2 * sizeof(double), cudaMemcpyHostToDevice); 
...
// KERNELS 
    ...
    reduce_without_s_mem<<<num_blocks, threads, num_steps * sizeof(double)>>>(d_steps, d_block_result, num_steps, *step, 1); 
    ...
    reduce_without_s_mem<<<1, num_blocks, num_blocks * sizeof(double)>>>(d_block_result, d_result, num_blocks, *step, 0); 
